...
struct SyncCenterView: View {
    ...
    var body: some View {
        List {
            Section("Complete iOS Backup") {
                Button { exportURL = TallyPortableBackupManager.exportURL(from: store) } label: {
        ...
        .sheet(item: $importPreview) { preview in
            BackupImportPreviewView(preview: preview) { mode in
                do {
                    try TallyPortableBackupManager.importBackup(from: preview.url, into: store, mode: mode)
                    switch mode {
                    case .mergeData:
                        importMessage = "Backup data merged. Your current appearance was preserved."
                    case .replaceEverything:
                        importMessage = preview.includesCosmetics ? "Backup restored, including cosmetic settings." : "Legacy backup restored. It did not contain accent or app icon settings."
                    case .appearanceOnly:
        ...
        .fileImporter(isPresented: $showingImporter, allowedContentTypes: [.json, Self.tallySyncType], allowsMultipleSelection: false) { result in
            handleImport(result)
        }
    }
    ...
    private func handleImport(_ result: Result<[URL], Error>) {
        ...
        case .success(let urls):
            guard let source = urls.first else { importMessage = "No backup file selected."; return }
            do {
                let scoped = source.startAccessingSecurityScopedResource()
                defer { if scoped { source.stopAccessingSecurityScopedResource() } }
                let data = try Data(contentsOf: source)
                let fileExtension = source.pathExtension.isEmpty ? "json" : source.pathExtension
                let local = FileManager.default.temporaryDirectory.appendingPathComponent("Tally_Import_\(UUID().uuidString).\(fileExtension)")
        ...
        NavigationStack {
            List {
                ...
                Section("Import Options") {
                    Button { onImport(.mergeData); dismiss() } label: { Label("Merge Data Only", systemImage: "plus.square.on.square") }
                    if preview.includesCosmetics {
                        Button { onImport(.appearanceOnly); dismiss() } label: { Label("Restore Appearance Only", systemImage: "paintbrush.pointed.fill") }
                    }
                    Button(role: .destructive) { onImport(.replaceEverything); dismiss() } label: { Label("Replace Everything", systemImage: "arrow.triangle.2.circlepath") }
                }
            }
                ...
                VStack(alignment: .leading, spacing: 24) {
                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        HStack {
                            ...
                            Spacer()
                            Button("Use Custom") { customHex = customColor.hexString(); accentRaw = StoredAccentColor.customValue }.buttonStyle(.borderedProminent)
                        }
                    }
                    .padding(18)
                    .background(store.theme == .oled ? Color(white: 0.035) : Color.clear, in: RoundedRectangle(cornerRadius: 22))
...
struct AppIconSettingsView: View {
    ...
    var body: some View {
        ...
        .tallyOLEDBackground().navigationTitle("App Icon")
    }

    @ViewBuilder
    private func iconPreview(_ icon: TallyIcon) -> some View {
        #if canImport(UIKit)
        if let path = Bundle.main.path(forResource: icon.previewName, ofType: "png"), let image = UIImage(contentsOfFile: path) {
            Image(uiImage: image).resizable().scaledToFit().frame(width: 64, height: 64).clipShape(RoundedRectangle(cornerRadius: 15, style: .continuous))
        } else if let image = UIImage(named: icon.previewName) {
            Image(uiImage: image).resizable().scaledToFit().frame(width: 64, height: 64).clipShape(RoundedRectangle(cornerRadius: 15, style: .continuous))
        } else {
            RoundedRectangle(cornerRadius: 15, style: .continuous).fill(icon.tint.gradient).frame(width: 64, height: 64).overlay(Image(systemName: "number").font(.title.bold()).foregroundStyle(.white))
        }
        #else
    ...
    private func setIcon(_ icon: TallyIcon) {
        ...
        UIApplication.shared.setAlternateIconName(icon.iconName) { [iconTitle = icon.title] error in
            Task { @MainActor in
                iconMessage = error?.localizedDescription ?? "Icon changed to \(iconTitle)."
            }
        }
        #endif
    }
    ...
    var body: some View {
        List {
            if filtered.isEmpty {
                ...
                ForEach(filtered) { counter in ArchivedCounterRow(counter: counter) }
            }
        }
        .tallyOLEDBackground().navigationTitle("Archive").searchable(text: $searchText, prompt: "Search archive")
    }
}
            ...
            List {
                Section {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Text("A major foundation update with stable folders, complete cosmetic backups, advanced sessions, reset scheduling, and native-feeling organization.").foregroundStyle(.secondary)
                    }.padding(.vertical, 8)
                }
                Section("v2.0") {
